...
from utilities.steering_wheel import SteeringWheelController
from carla_utils.map_utils import BeVWrapper
from carla_utils.bev_utils import get_birdview, visualize_birdview
...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, host, port, town, fps, obs_sensor, obs_res, view_res, reward_fn, action_smoothing, bev_config, allow_render=True, allow_spectator=True, enable_takeover=True):
        ...
        # Setting birdview image
        self._bev_wrapper = BeVWrapper(bev_config)

        # Core RL: Action, Observation, Reward
        self.actions = CarlaActions()
        self.observations = CarlaObservations(self.obs_height, self.obs_width, self.obs_sensor)
        ...
        # Number of traffic agents to spawn
        self._n_pedestrians = 30
        self._n_vehicles = 30
        ...
        try:
            ...
            # Spawn traffic with pedestrians
           ...
           # Spawn collision and Lane invasion sensors
            ...
            if 'rgb' in self.obs_sensor:
                ...
                self.cam.set_attribute('image_size_x', f'{self.obs_width}')
                self.cam.set_attribute('image_size_y', f'{self.obs_height}')
                self.cam.set_attribute('fov', '90')
                bound_x = self.vehicle.bounding_box.extent.x
                transform_front = carla.Transform(carla.Location(x=bound_x, z=1.0))
                self.sensor_front = self.world.spawn_actor(self.cam, transform_front, attach_to=self.vehicle)
                self.sensor_front.listen(self._set_observation_image) 
            elif 'semantic' in self.obs_sensor:
                self.cam = self.world.get_blueprint_library().find('sensor.camera.semantic_segmentation')
                self.cam.set_attribute('image_size_x', f'{self.obs_width}')
                self.cam.set_attribute('image_size_y', f'{self.obs_height}')
                self.cam.set_attribute('fov', '90')
                bound_x = self.vehicle.bounding_box.extent.x
                ...
                self.sensor_front = self.world.spawn_actor(self.cam, transform_front, attach_to=self.vehicle)
                self.sensor_front.listen(self._set_observation_image) 
            elif 'birdview' in self.obs_sensor:
                self._bev_wrapper.init(self.client, self.world, self.map, self.vehicle, self.display)
            else:
            ...
            if self.enable_takeover:
                self.controller = SteeringWheelController(disable=True)
            else:
    ...
    def step(self, action):
        ...
        if self.controller is not None:
            try:
                human_action = self.controller.process_input(self.get_vehicle_lon_speed())
        ...
        if self.takeover is False and action is not None:
            ...
            if self.control.throttle < 0:
                self.control.brake = self.control.throttle
        ...
        self.world.tick()
        # Update BEV
        self._bev_wrapper.update_waypoints(self.route_waypoints)
        ...
        self.processed_obs = self.postprocess_obs(self.observation)
    ...
    def _spawn_pedestrians(self):
        ...
        while peds_spawned < self._n_pedestrians:
            ...
            for spawn_point in spawn_points:
                ...
                if walker_bp.has_attribute('speed'):
                    ...
                    logging.error("[SIMULATOR] Walker has no speed")
            ...
            for i in range(len(results)):
                if results[i].error:
                    logging.error('[SIMULATOR] Walker ', results[i].error)
                else:
                    peds_spawned += 1
            ...
            for result in self.client.apply_batch_sync(batch, True):
                if result.error:
                    logging.error('[SIMULATOR] Walker controller ', result.error)
                else:
        ...
        self.world.set_pedestrians_cross_factor(pedestrians_crossing)
    ...
    def get_takeover_cost(self, human_action, agent_action):
        takeover_action = self.safe_clip(np.array(human_action), -1, 1)
        agent_action = self.safe_clip(np.array(agent_action), -1, 1)
